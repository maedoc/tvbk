struct sim_t {
  const uint32 nnode, nsvar, ntime, maxdelay, h2, ntavg;
  const float *weights;
  const uint32 *idelays;
  const float cv, dt, progress_period;

  const float *G; // 8
  const float *K_bath; // nnode, 8

  // workspace
  float *states; // nsvar, nnode, 8
  float *history; // ncvar, nnode, idelays.max()+2, 8
  float *tavg;     // (ntavg, nsvar, nnode, 8)
};

#define NSVAR 5

static void dfun(float dx[NSVAR], const float x[NSVAR], const float c[1], const float p[1]) {
...
{
  uniform uint32 nn = s->nnode;
  uniform uint32 ipp = floor(s->progress_period / s->dt);
  uniform uint32 tpp = s->ntime / s->ntavg;
  uniform float * uniform ta = s->tavg;
  uniform float rtpp = 1.f / tpp;
  ...
  for (uniform uint32 t = 0; t < s->ntime; t++) {
    ...
    foreach (l = 0 ... 8) {
      for (uint32 i = 0; i < nn; i++) {
        varying float dx1[NSVAR] = {0}, dx2[NSVAR] = {0}, cx[1] = {0.f},
                      xi[NSVAR] = {0}, x[NSVAR] = {0}, p[1] = {s->K_bath[i*8 + l]};

        for (uniform uint32 j = 0; j < nn; j++) {
          float w = s->weights[j * nn + i];
          if (all(w == 0.f))
            continue;
          uint32 d = s->idelays[j * nn + i];
          d = (t - 1 - d + s->h2) & (s->h2 - 1); // faster modulo by power of 2
          cx[0] += s->G[l] * w * s->history[(j * s->h2 + d)*8 + l];
        }

        for (uniform uint32 v = 0; v < NSVAR; v++)
          x[v] = s->states[(v * nn + i)*8 + l];
        ...
        for (uniform uint32 v = 0; v < NSVAR; v++) {
          ...
          s->states[(v * nn + i)*8 + l] = x[v];
          ta[(v * nn + i)*8 + l] += x[v];
        }
        s->history[(i * s->h2 + (t & (s->h2 - 1)))*8 + l] = x[0];
      }
  ...
  foreach (i = 0 ... (s->ntavg * NSVAR * nn * 8))
    s->tavg[i] *= rtpp;
}
