...
export void sim_run(const uniform sim_t * uniform s)
{
  ...
  for (uniform uint32 t = 0; t < s->ntime; t++) {
    ...
    foreach (i = 0 ... s->nnode)
    {
      ...
      // cx[0] += s->G*s->weights[j*s->nnode + i]*s->states[j];

      // in tvb, current state is at (t-1)%horizon
      // and delayed state is at (t-1-idelay+horizon)%horizon
      // t starts at 0, so initial conditions are at horizon-1
      for (uniform uint32 j = 0; j < s->nnode; j++) {
      ...
      for (uniform uint32 v = 0; v < NSVAR; v++) {
        ...
        s->raw[t * NSVAR * s->nnode + v * s->nnode + i] = x[v];
      }
