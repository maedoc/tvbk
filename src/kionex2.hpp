/* KIonEx model - Potassium Ion Exchange mean-field model - Version 2 (with
 * J*r*(E-V) term in dV/dt) */
...
#pragma omp simd
    for (int i = 0; i < width; i++) {
      ...
      auto m_inf = [Cmna, DCmna](float V_val) {
        return 1.0f / (1.0f + expf((Cmna - V_val) / DCmna));
      };
      auto n_inf = [Cnk, DCnk](float V_val) {
        return 1.0f / (1.0f + expf((Cnk - V_val) / DCnk));
      };
      auto h = [](float n_val) {
        return 1.1f - 1.0f / (1.0f + expf(-8.0f * (n_val - 0.4f)));
      };
      ...
      float I_Na = (g_Nal + g_Na * m_inf(V) * h(n)) *
      ...
      dx[i + width] =
          (V <= Vstar) ? Vdot - R_minus * x_var * x_var + eta +
                             J * r * (E - V) + (R_minus / M_PI) * c[i] * (E - V)
