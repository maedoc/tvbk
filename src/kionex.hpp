...
struct kionex {
  ...
  dfun(float *__restrict dx, const float *__restrict x, const float *__restrict c, const float *__restrict p)
  {
    ...
    for (int i=0; i<width; i++) {
      ...
      float I_Na = (g_Nal + g_Na * m_inf(V) * h(n)) * (V - 26.64f * logf(Na_o/Na_i));
      ...
      dx[i+width] = (V <= Vstar) ?
                   Vdot - R_minus*x_var*x_var + eta + (R_minus/M_PI)*c[i]*(E-V) :
                   Vdot - R_plus*x_var*x_var + eta + (R_minus/M_PI)*c[i]*(E-V);
