...
static void heun_step(const cxb<width> &cx, float *__restrict states,
                      float *__restrict zscl, const float *__restrict cx1,
                      const float *__restrict cx2,
                      const float *__restrict params, const uint32_t i_node,
                      const uint32_t i_time, const float dt, uint64_t *seed) {
  constexpr uint8_t nsvar = model::num_svar;
  const uint32_t num_node = cx.num_node, horizon = cx.num_time;
  float x[nsvar * width], xi[nsvar * width] = {}, dx1[nsvar * width] = {},
                                     dx2[nsvar * width] = {};
  float z[width] = {};
...
#pragma clang loop unroll(full)
  for (int svar = 0; svar < nsvar; svar++) {
    ...
    sheunpred<width>(x + svar * width, xi + svar * width, dx1 + svar * width, z,
                     dt);
  }
...
#pragma clang loop unroll(full)
  for (int svar = 0; svar < nsvar; svar++) {
    scaledrandn<width>(z, seed, zscl + svar * width);
    sheuncorr<width>(x + svar * width, dx1 + svar * width, dx2 + svar * width,
                     z, dt);
  }
